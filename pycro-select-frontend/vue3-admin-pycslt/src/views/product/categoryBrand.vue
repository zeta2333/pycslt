...
import { GetCategoryBrandPageList, SaveCategoryBrand, UpdateCategoryBrand, DeleteCategoryBrandById } from '@/api/product/categoryBrand.js'
...
const deleteById = (row) => {
    ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
        ...
        const { code } = await DeleteCategoryBrandById(row.id)
        if (code === 200) {
